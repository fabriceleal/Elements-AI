...
(setq diff_sin_rule `(
	differentiate
	(d (  ,(lambda (x) (and (match '((? head) (* args)) x) (equal head 'sin))) body  ))
	(list '* -1 (cons 'cos args))
	diff_sin_rule
	) )
...
; d(f(g(x))) => u = g(x), d(f/u).d(g(x)/x)
(setq diff_chain_rule `(
	differentiate
	(d (,(lambda(x) (match  `((? head) (,(lambda(x) (listp x)) tail)) x)) all))
	;(* (d (list head)) (d tail))
	(list '* (list 'd (cons head (cdr tail))) (list 'd tail))
	diff_chain_rule	
	) )
